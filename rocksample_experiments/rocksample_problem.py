"""
Copy of the implementation of the rocksample problem from pomdp_py library, but with some modifications:
Sample action now contains the rock_id that is sampled. This change is in order to be able to track the rocks that
are sampled in order to implement the preferred actions similarily to the POMCP paper.
...
class SampleAction(Action):
    def __init__(self, position):
        # save position to quickly check where sampled in history
...
class RSPolicyModel(pomdp_py.RolloutPolicy):
    ...
    def __init__(self, n, k):
        self._check_actions = set({CheckAction(rock_id) for rock_id in range(k)})
