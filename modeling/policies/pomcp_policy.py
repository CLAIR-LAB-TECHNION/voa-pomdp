...
class POMCPPolicy(AbstractPolicy):
    ...
    def sample_action(self, belief=None, history=None):
        assert belief is None and history is None, "POMCP maintains it own belief. Instead of passing belief to " \
                                                   "planning need to call update(action, observation)"
    ...
    def update(self, action, observation):
        # POMCP updates agent belief automatically in planner.update
    ...
    def get_belief(self):
        return deepcopy(self.agent.cur_belief)
